...
void App::Startup()
{
	...
	//Test JobSystem
	g_theJobs = new JobSystem();
	g_theJobs->CreateWorkerThreads(12);
	...
	g_theEvent->SubscribeToEvent("warp", WrapMap);
}
...
App::~App()
{
	...
	std::map<std::string, SpriteSheet*>::iterator itr;
	for (itr = MapMaterialType::s_spriteSheet.begin(); itr != MapMaterialType::s_spriteSheet.end(); ++itr)
	{
		delete itr->second;
	}
...
void App::BeginFrame()
{
	...
	//test job system
	//ExampleJob* job1 = new ExampleJob();
	//g_theJobs->PostJob(job1);
}
...
void App::EndFrame()
{
	...
	//test job system
	g_theJobs->ClaimAndDeleteAllCompletedJobs();
}
...
void App::UpdateFromKeyboard()
{
	...
	{
 		JobFindLargestPrime* newJob = new JobFindLargestPrime(250000);
 		g_theJobs->PostJob(newJob);
	}
