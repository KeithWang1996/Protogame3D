...
float4 FragmentFunction(VertexToFragment_t input) : SV_Target0 // semeantic of what I'm returning
{
   ...
   float3 finalColor = lerp(startColor, endColor, grayScaleFactor);
   float3 overflow = max(finalColor - float3(1.0f, 1.0f, 1.0f), float3(0.0f, 0.0f, 0.0f));
   return float4(overflow, color.a);
}